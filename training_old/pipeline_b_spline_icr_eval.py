"""
training/pipeline_b_spline_icr_eval.py
────────────────────────────────────────
Extract -> filter -> EQ -> fit B spline -> spline-smooth measured params
-> train NN without B (ICR eval) -> export hybrid (B from spline)
-> spline-fix NN notes.

Difference from smooth-icr-eval:
    - After extraction a BSplneFitter is fitted from measured notes.
      B is a physically velocity-independent parameter (string stiffness);
      fitting it as a smooth spline over MIDI eliminates the dominant source
      of multi-task loss (~5–8× other terms in standard training).
    - NN is trained with ProfileTrainerNoB: B excluded from targets, gradient,
      and optimizer state.  The 10 remaining heads receive full gradient
      capacity.
    - Profile generation uses generate_profile_exp_no_b: B filled from spline
      (one value per MIDI, reused for all 8 velocity layers).

Output files:
    <out_path>                           final hybrid + spline-fixed bank
    <stem>-pre-smooth.json               measured-only simple export
    <stem>-pre-smooth-spline.json        spline-smoothed measured params

Call via run-training.py or import directly:
    from training.pipeline_b_spline_icr_eval import run
    model, b_fitter, out_path = run(bank_dir, out_path, icr_exe="build/bin/Release/ICR.exe")
"""
...
from training.modules.profile_trainer_no_b      import (
    ProfileTrainerNoB,
    build_dataset_exp_no_b,
...
def run(
    ...
    sr:            int   = 48000,
    auto_anchors:  int   = 12,
    b_stiffness:   float = 2.0,
) -> tuple:
    """
    B-spline ICR-eval pipeline:
    Extract -> filter -> EQ -> fit B spline -> spline-smooth measured params
    -> train NN without B (ICR early-stop) -> export hybrid -> spline-fix NN notes.
    ...
    Args:
        ...
        b_stiffness:  Stiffness of the B spline (higher = smoother curve).

    Returns:
        (model, b_fitter, out_path) — trained model, fitted B spline, final bank path.
    """
    ...
    measured = {k: v for k, v in params["samples"].items()
                if not v.get("_interpolated")}

    # ── Step 2: Fit B spline from measured notes ──────────────────────────────
    print(f"\n[2/5] Fitting B spline from {len(measured)} measured notes "
          f"(stiffness={b_stiffness})...")
    b_fitter = BSplneFitter(stiffness=b_stiffness).fit(measured)

    # ── Step 3: Export simple bank + spline-smooth measured params ────────────
    print(f"\n[3/5] Exporting simple bank -> {pre_path}")
    SoundbankExporter().from_params(params, pre_path)

    print(f"  Spline-smoothing measured params (auto_anchors={auto_anchors})...")
    ...
    print(f"\n[4/5] Training NN without B on smooth params (max {epochs} epochs)...")
    ...
    # ── Step 5: Export hybrid (B from spline) + spline-fix NN notes ──────────
    print(f"\n[5/5] Generating hybrid profile (B from spline) + fixing NN notes...")
    hybrid_path = out_path.replace(".json", "-hybrid-raw.json")

    ds_exp       = build_dataset_exp_no_b(measured)
    all_samples  = generate_profile_exp_no_b(
        model, ds_exp, b_fitter,
        midi_from=21, midi_to=108, sr=sr,
        orig_samples=measured,
    )
    hybrid_params = {**smooth_params, "samples": all_samples}
    SoundbankExporter().from_params(hybrid_params, hybrid_path)
