...
## Experiment — comparing the three workflows

All three pipelines run with the same settings (`N_EVAL_MIDI=24`, `eval_vels=[0,2,4,6,7]`)
and produce directly comparable soundbanks for A/B listening.

### What each pipeline tests

| | **icr-eval** | **smooth-icr-eval** | **full-spline-icr-eval** |
|---|---|---|---|
| NN training targets | raw extracted params | spline-smoothed measured | spline-smoothed measured |
| NN notes after export | as generated by NN | replaced by spline | replaced by spline |
| Harmonic count (K_valid) | as generated by NN | as generated by NN | extended to max measured |
| Velocity eval coverage | [0,2,4,6,7] | [0,2,4,6,7] | [0,2,4,6,7] |
...
2. **Does extending harmonics help?** *(smooth-icr-eval vs full-spline-icr-eval)*
   NN-generated notes in the extrapolation zone have fewer partials than measured notes.
   `full-spline-icr-eval` fills them in via spline. Does the extra harmonic content
   improve the perceived tone of those notes?

3. **How much does the spline post-processing alone contribute?**
   Both `smooth-icr-eval` and `full-spline-icr-eval` replace NN notes with spline
   values after export. Comparing to `icr-eval` (no post-processing) isolates the
   spline contribution independently of the NN training quality.
...
soundbanks/params-vv-rhodes-full-spline-icr-eval.json
```

Load all three into the sound editor and compare by playing the same notes across
