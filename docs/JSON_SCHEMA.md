...
| `spectral_eq` | object | per-(midi,vel) | EQFitter (LTASE měření) | ✗ ignoruje | ✓ (EQ editor) | ✗ | Surová EQ křivka `{freqs_hz: [...], gains_db: [...]}` použitá pro biquad fit; uložena pro re-fit po editaci |
...
|---|---|---|---|
| `B` | ✓ (BSplneFitter) | | |
| `f0_hz`, `attack_tau`, `A_noise`, `rms_gain`, `phi_diff` | | ✓ | |
...
|---|---|
| **C++ `loadBankJson`** | `midi`, `vel`, `phi_diff`, `attack_tau`, `A_noise`, `rms_gain`, `f0_hz`, `B`, `_interpolated`, `partials[k, f_hz, A0, tau1, tau2, a1, beat_hz, phi]`, `eq_biquads[b, a]` |
| **C++ `setNoteParam`** | `f0_hz`, `attack_tau`, `A_noise`, `rms_gain`, `phi_diff`, `B` (→ všechny vel) |
