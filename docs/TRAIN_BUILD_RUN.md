...
params-<banka>.json              fyzikální parametry per nota × parcial
    │
    ▼  training/find_outliers.py --drop
...
params-<banka>-nn.json           NN-smoothed profil 88×8 pozic
    │
    ▼  training/closed_loop_finetune.py --mode finetune   (volitelné)
profile-<banka>-ft.pt            doladěný model po MRSTFT fine-tuningu
    │
    ▼  training/train_instrument_profile.py --epochs 0
soundbanks/params-<banka>-ft.json   ICR-ready JSON pro GUI/CLI/RenderServer
    │
    ▼  training/closed_loop_finetune.py --mode global     (volitelné)
params-<banka>-ft.synth_config.json   beat_scale + noise_level optimalizace
    │
    ▼  training/export_soundbank_params.py                (alternativní cesta)
training/params-piano-soundbank.json  přímý export soundbank → ICR s EQ fitováním
```
...
**Skript:** `training/extract_params.py`

```bash
python -u training/extract_params.py \
    --bank    "C:/SoundBanks/IthacaPlayer/ks-grand" \
    --out     training/params-ks-grand.json \
    --workers 4
```
...
**Log:** `training/runtime-logs/extract-params-log.txt`

---

## 4. Krok 2 — Filtrace outlierů

**Skript:** `training/find_outliers.py`

```bash
# Vizuální inspekce — zobrazí histogram s outlieri
python training/find_outliers.py \
    --params training/params-ks-grand.json \
    --z 10 --plot --feature B
...
**Skript:** `training/compute_spectral_eq.py`

```bash
python -u training/compute_spectral_eq.py \
    --params  training/params-ks-grand.json \
    --bank    "C:/SoundBanks/IthacaPlayer/ks-grand" \
    --workers 4
```
...
**Log:** `training/runtime-logs/spectral-eq-log.txt`

---

## 6. Krok 4 — Surrogate NN trénink

**Skript:** `training/train_instrument_profile.py`

```bash
python -u training/train_instrument_profile.py \
    --in         training/params-ks-grand.json \
    --out        training/params-ks-grand-nn.json \
    --model      training/profile.pt \
    --epochs     1800 \
    --eval-every 10
```
...
**Výstupy:**
- `training/profile.pt` — `{"state_dict": ..., "hidden": 64, "eq_freqs": [...]}`
- `training/params-ks-grand-nn.json` — 704 vzorků, originály zachovány

---

## 7. Krok 5a — MRSTFT fine-tuning

**Skript:** `training/closed_loop_finetune.py --mode finetune`
...
```bash
python training/closed_loop_finetune.py \
    --mode         finetune \
    --model        training/profile.pt \
    --out          training/profile-finetuned.pt \
    --bank         "C:/SoundBanks/IthacaPlayer/ks-grand" \
    --epochs       200 \
    --lr           3e-4 \
    --batch-size   8 \
    ...
    --log          training/runtime-logs/finetune.log
```
...
```bash
python training/closed_loop_finetune.py \
    --mode finetune --model training/profile.pt \
    --bank "C:/SoundBanks/IthacaPlayer/ks-grand" \
...
python training/closed_loop_finetune.py \
    --mode       global \
    --model      training/profile-finetuned.pt \
    --bank       "C:/SoundBanks/IthacaPlayer/ks-grand" \
    --opt-params beat_scale,noise_level \
    --epochs     100 \
    --lr         0.05 \
    --log        training/runtime-logs/finetune-global.log
```

**Výstup:** `training/profile-finetuned.synth_config.json`
...
`training/train_pipeline.py` spustí kroky 1–6 jediným příkazem:
...
```bash
python training/export_soundbank_params.py \
    --soundbank soundbanks/params-ks-grand-ft.json \
    --out       training/params-piano-soundbank.json \
    --sr        44100 \
    --duration  3.0 \
    --target-rms 0.06 \
    --rng-seed  0
...
| `--out` | `training/params-piano-soundbank.json` | Výstupní ICR JSON |
...
Soubory mají příponu `.json`, formát `piano_core_v1`. Jsou načítány C++ třídou `PianoCore::load()` (přes `nlohmann/json`). Výchozí umístění: `soundbanks/` nebo `training/`.
...
| Target | Cesta | Popis |
|---|---|---|
| `ICRGUI` | `build/bin/Release/ICRGUI.exe` | GUI s Dear ImGui, MIDI vstup, real-time audio |
| `IthacaCoreResonator` | `build/bin/Release/ICR.exe` | Headless CLI, real-time audio |
...
```bat
build\bin\Release\IthacaRenderServer.exe soundbanks/params-ks-grand-ft.json --port 9876 --log training/runtime-logs/render-server.log
```
...
```bash
python training/closed_loop_finetune.py --mode finetune --model training/profile.pt \
